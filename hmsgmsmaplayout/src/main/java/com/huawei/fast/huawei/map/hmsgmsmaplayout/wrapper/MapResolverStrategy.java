...
public enum MapResolverStrategy {
    ...
    static Fragment useRequiredMapFragment(MapResolverStrategy strategy, Context context)
        ...
        switch (strategy){
            case FORCE_GOOGLE: return getGoogleFragment();
            case FORCE_HUAWEI: return getHuaweiFragment();
    ...
    static FrameLayout useRequiredMapView(Context context, AttributeSet attr, int defStyleAttr, MapResolverStrategy strategy)
            throws MapResolverException {
        switch (strategy){
            case FORCE_GOOGLE: return getGoogleMapView(context,attr,defStyleAttr);
            case FORCE_HUAWEI: return getHuaweiMapView(context,attr,defStyleAttr);
            case GOOGLE_THEN_HUAWEI: {
                if (ApiAvailability.isGoogleAvailable(context)) return getGoogleMapView(context,attr,defStyleAttr);
                if (ApiAvailability.isHuaweiAvailable(context)) return getHuaweiMapView(context,attr,defStyleAttr);
            }
            case HUAWEI_THEN_GOOGLE: {
                if (ApiAvailability.isHuaweiAvailable(context)) return getHuaweiMapView(context,attr,defStyleAttr);
                if (ApiAvailability.isGoogleAvailable(context)) return getGoogleMapView(context,attr,defStyleAttr);
